...
public class F_Practice {
    public static void main(String[] args) {
        // [1] 학생 이름과 정수를 저장할 Map 생성
        Map<String, Integer> studentScores = new HashMap<>();

        studentScores.put("김지지", 90);
        studentScores.put("김동동", 80);
        studentScores.put("권지지", 95);
        studentScores.put("박현현", 85);
        studentScores.put("김승승", 100);
...
/*  Predicate<T> (조건 판단): score >= 70 판단
    Function<T, R> (변환 처리): 점수를 등급으로 변환
    Consumer<T> (값 소비(출력)): 학생 정보 출력, 구분선 출력
    Supplier<T> (값 제공(입력 없음)): 무작위 버너스 점수 생성
 */
