...
/*  === 메서드 참조 ===
    ...
    4. 임의 객체의 인스턴스 메서드
    - 특정 객체가 아닌 여러 객체에 공통된 인스턴스 메서드를 사용 할 떄
...
public class G_MethodRef {
    public static void main(String[] args) {
        ...
        // 2. 인스턴스 메서드 참조
        // cf) 문자열 객체의 메서드 사용
        //      - Object를 상속받는 String 클래스는 선언 ㅅ ㅣ객체 생성
        ...
        // 3. 생성자 참조
        // 1) 람다 표현식
        Supplier<Person> personLambda = () -> new Person("이승승");
        Supplier<Person> personLambda2 = () -> new Person();
        // 2) 메서드 참조
        // : 매개변수가 없는 생성자일 경우에만 활용 가능!!!
        Supplier<Person> personLambdaRed = Person::new; // 매개변수가 없는 생성자를 호출

        Person p1 = personLambda.get();
        Person p2 = personLambdaRed.get();
