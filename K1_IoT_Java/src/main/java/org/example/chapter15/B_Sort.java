...
/*  1. 불변성 데이터: Stream API 정렬 사용
    ...
    2. 원본 리스트 자체 정렬
        ...
        목적) 원본 데이터를 적접 수정해서 정렬 순서를 유지해야 하는 경우
        EX) 캐시된 리스트, 재사용 리스트, 통계 계산용 리스트 등
            >> 데이터 베이스나 외부 API에서 자주 조회되는 데이터를 미리 리스트에 저장해두고
                , 필요 시 다시 원본을 가져오지 않고 빠르게 읽어오는 용도

    3. Comparator 추출로 정렬 기준 재사용
        목적) 정렬 기준이 자주 반복되거나 복잡한 로직일 때 재 사용성르 높이기 위함
      */
...
class UserComparators {
    ...
    // : 테스트 용이성, 유지보수성 향상
    // - 실무 프로제그에서는 ComparatorUtils, SortConstants 클래스에 모아서 정의
}
public class B_Sort {
