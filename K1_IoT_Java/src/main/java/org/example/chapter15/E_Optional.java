...
public class E_Optional {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            optional3.orElseThrow(() -> new Error("값이 없을 경우예외 발생")); // 런타임
            // optional3.orElseThrow(() -> new Exception("예외 발생")); // 컴파일
        } catch (Error error) {
            System.out.println("예외가 발생하였습니다: " + error.getMessage());
            // 예외가 발생하였습니다: 값이 없을 경우예외 발생
        }
        ...
        // 1) 필드에 사용 X
        // 2) 메서드 매개변수로 사용 X
        //      >> 반환값으로 사용 O (메서드 호출 결과가 null일 수 있는 경우에만)
        // 3) 값이 반드시 존재해야하는 경우 사용 X
