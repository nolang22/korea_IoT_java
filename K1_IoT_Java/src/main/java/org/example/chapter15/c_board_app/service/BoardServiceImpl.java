...
public class BoardServiceImpl implements BoardService{
    ...
    public List<BoardResponseDto> findAllBoards() {
        ...
        List<BoardResponseDto> boardResponseDtos = boardList.stream()
                // .map(board -> BoardResponseDto.formEntity(board))
                .map(BoardResponseDto::formEntity)
    ...
    public BoardResponseDto findBoardById(Long id) {
        ...
        // : 변환 작업을 할 떄 내부 데이터가 null 인 경우 Exception 발생
        BoardResponseDto boardResponseDto = board.map(BoardResponseDto::formEntity)
